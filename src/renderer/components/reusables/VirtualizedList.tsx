 ...
 * The list displays enough items to fill the window + some extra for scroll performance. 
 * As the list is scrolled out of the window, the items are moved from the outgoing side 
 * and placed into the incoming side with CSS positioning, giving the illusion of
 * the infinite scroll.
 * 
 * The list is scrolled by the scroll event within its bounds and the optional property
 * scrollTop if an outside scrolling event wishes to control this list.
 */
export default function VirtualizedList(props: VirtualizedListProps) {
    ...
    /**
     * Control the list container scroll top by the scrollTop property.
     */
    useLayoutEffect(() => {
    ...
    useLayoutEffect(() => {
        if (props.count > 0) {
            const position = Math.max(0, Math.floor(scrollTop / props.rowHeight) 
                - props.overflowCount)
